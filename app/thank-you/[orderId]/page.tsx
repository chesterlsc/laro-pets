...
export default async function ThankYouPage({ params, searchParams }: { params: Promise<{ orderId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const unitPrice = order.items.unitPrice ?? tier.price;
  const gaItems = [
    { item_id: product.sku[order.items.prints[0]], item_name: tier.name, price: unitPrice, quantity: 1 },
    ...(order.items.extraRefills > 0 ? [{ item_id: 'LARO-REFILL-3', item_name: '3-feather refill pack', price: order.items.refillPrice ?? product.prices.refill, quantity: order.items.extraRefills }] : []),
  ];
  const next = [
    cod ? 'We’ll text you to confirm, then pack and hand it to the courier.' : 'We pack your order and hand it to the courier.',
    `You get a tracking number by SMS within ${placeholders.deliveryDaysMetroManila} days (Metro Manila) or ${placeholders.deliveryDaysProvinces} days (provinces).`,
  ];
  ...
  return (
    <main className="py-10 xl:py-16">
      <Container className="max-w-[760px]">
        {!confirming && <PurchaseTracker orderNo={order.order_no} value={order.total} items={gaItems} />}
        <div className="flex flex-col gap-6 rounded-card border border-border bg-white p-6 xl:p-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-[16px] text-muted">We&apos;ll text the number ending in {order.customer.mobile.slice(-4)} with updates.</p>
          </div>
          ...
          {confirming && <p role="status" className="flex items-center gap-2 text-[15px] font-bold text-muted"><Icon name="lock" size={18} />Confirming your payment… refresh in a minute. If you closed the payment page, your order is on hold — nothing has been charged.</p>}
          ...
          <div className="flex flex-col gap-3 rounded-inner bg-bg p-5">
            <div className={row}>
              ...
              <span className="font-bold">{peso(unitPrice)}</span>
            </div>
            {order.items.extraRefills > 0 && <div className={row}><span>Extra refill packs × {order.items.extraRefills}</span><span className="font-bold">{peso(order.subtotal - unitPrice)}</span></div>}
